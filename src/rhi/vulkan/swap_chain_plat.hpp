// rhi/vulkan/swap_chain_plat.hpp
...
struct SwapChainPlat {
    ...
    SwapResolveTarget AcquireForFrame() {
        ...
        // The vk swap target carries a pointer to the OWNING SwapChain (not
        // SwapChainPlat) -- callers go through sc->plat.X. The owner pointer
        // is patched by SwapChain::AcquireForFrame on the way out.
    ...
    // Neutral teardown entry (matches Metal SwapChainPlat::Deinit). Device
    // must still be alive (call before ResourceManager::Deinit destroys it).
    void Deinit() {
