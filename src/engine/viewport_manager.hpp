...
//
// Viewport ownership state: the Viewport pool, the per-slot id/name tables,
// the active viewport index/count, and the vpN wire-name counter. Engine's
// viewport + camera + click-focus systems operate on it.
//
...
// One pane of a composition -- draw a source viewport's resolved target
// (color or depth) into rect_ndc of the swap. Data, not a mode flag: the
// composite pass iterates these instead of branching. [N-node] this IS
// "node consumes another node's framebuffer" in miniature; the shape is
// general (any viewport -> any rect) though only the render.nestedGraph op
// installs entries today (the depth-PIP = two entries).
...
struct ViewportManager {
    ...
    int active_index = 0;
    int active_count = 1;  // runtime-live count, default 1
    ...
    // Presentation panes. Empty -> the default per-viewport layout_rect
    // composite; non-empty -> the composite pass draws exactly these.
