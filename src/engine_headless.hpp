...
#include "scene/component_type.hpp"  // ComponentType
...
// Selection / highlight / pick. Document-side state on Engine; this facade
...
// Viewport lifetime + tile layout (NDC 0..1 over the swap pane).
// OpenViewport returns the engine-assigned monotonic name counter
// ("vp{N}" without prefix; cast to int so -1 signals capped). Names are
// never reused.
int OpenViewport(Engine* engine);
bool CloseViewport(Engine* engine);
// Int-slot variant; the wire-name path is SetViewportLayoutByName.
...
// Poll the most recent resolved pick. resolved=false until the engine
// has run a frame after RequestPick and resolved it.
struct PickResultExport {
    ...
    uint32_t id = 0;
    // Picking is a CPU ray-cast, so raw == id.
    uint32_t raw = 0;
};
PickResultExport ConsumePickResult(Engine* engine);

// Spawn one hero entity in active_scene_ from a pre-loaded scene
...
bool SetEntityTransform(Engine* engine, uint32_t entity_int,
                         float x, float y, float z, float scale);
// Entity ops. scene_index -1 = active. TRS as plain float arrays so
...
bool SetEntityName(Engine* engine, int scene_index, uint32_t entity,
                   const std::string& name);
// Generic component ops. type = ComponentType; add/get are typed
...
bool GetEntityRenderable(Engine* engine, int scene_index, uint32_t entity,
                         uint32_t& layer_mask, uint32_t& flags);
// cairns.time.get: read-only sim-clock snapshot (never ticks).
void TimeNow(Engine* engine, double& time, double& dt, uint64_t& frame);
// Camera enumeration + viewport camera-entity binding.
struct CameraEntry {
    uint32_t entity = 0;
    bool is_main = false;
};
...
// Per-Engine synthetic scene id counter (deliberately not a global).
uint64_t NextSceneId(Engine* engine);

// Drop every resident prefab + DeferFree their GPU resources through
// the fenced deferred-deletion ring (no WaitIdle stall). Calls
// ClearActiveScene internally so the post-state is "engine empty, ready
// for fresh loads." Returns the number of prefabs dropped (0 if empty).
uint32_t UnloadAllPrefabs(Engine* engine);

// Hot-reload the resident prefab matching |path|. Re-parses
...
// Hot-reload a pipeline by logical name (anim_eval / skin /
...
// Loader instruments from the last LoadPrefabBatch call.
LoadTrace LastLoadTrace(Engine* engine);
LoaderCounters Counters(Engine* engine);
// Validation report from the last LoadPrefabBatch call.
ValidationReport LastValidationReport(Engine* engine);

// Runtime batch load (drain + load + re-upload anim tables). Load
// APPENDS, never replaces. Returns {first_prefab_idx, count}.
...
// Single-path load. The JS catalog loops + calls this once
...
// JS-driven golden primitives. The SCENARIO choreography lives in JS
...
// Instantiate `prefab_count` prefabs starting at `first_prefab_idx`
...
// Debug pair asserting the load-APPENDS-never-replaces contract.
uint32_t DebugSnapshotPrefabHandles(Engine* engine);
uint32_t DebugAssertAppendOnly(Engine* engine);

// Invariant check across the LoadPrefabBatch manifest contract.
...
// Deterministic load-twice check. Runs RuntimeLoadGlbs twice
