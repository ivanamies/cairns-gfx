...
## `984dae1+` (2026-06-14) — 500 actors, 100 distinct GLBs, macOS arm64 (M-class)

Workload: 100 distinct LoL-skin GLBs loaded via cairns.prefab.load,
spawnTotal(500) — grid of ~23x22, all skinned, animated. Window
2560x1440. CAIRNS_AGENT_STDIN=1 piped via FIFO; idle ~6 s post-spawn
before steady-state read.

ANIM-CAP=1024, SKIN-FAIL=0 on both backends. Animation visible in
metal; vk render output corrupted (see notes).

### metal-release (Release, build/metal-release)
...
| present_wait | 0.00 |
| skin_eval | 0.83 |
| skinning_compute | 14.80 |

FPS: 48. Screenshot: `tmp/_perf_metalrel.png` — 500 actors visible animated.

### vk-release (Release, build/vk-release)
...
| fence_wait | 0.00 |
| skin_eval | 0.03 |
| skinning_compute | 12.10 |

FPS: 48. Screenshot: `tmp/_perf_vkrel.png` — rendered output is
fractured / corrupted triangles. Numbers recorded; functional render
fail tracked as task #327.

### metal-debug (Debug, build/metal) — control
...
| swap | 0.21 |
| skin_eval | 2.11 |
| skinning_compute | 14.54 |

### CPU 20–22 ms breakdown

CPU frame ≈ gpu_frame on both backends — main thread is gated on
render-thread/GPU throughput. kFramesInFlight=3 buffer drains because
gpu_frame > 16.6 ms (60 Hz vsync window).

Present-wait is 0 across the run (the cv.wait at engine.hpp:~2949
was replaced by `std::deque<int32_t> present_queue_` drained in
submit order, 19e9149).

### Skinning compute notes (per #326)

500 actors × ~30k verts/actor = 15M threads at workgroup size 64.

Back-of-envelope (M1/M2 GPU):
- bandwidth: 40 B/vert × 15M = 600 MB / ~120 GB/s ≈ 5 ms
- ALU: 50 fp32 ops/vert × 15M ÷ ~1600 GFLOPS ≈ 0.5 ms
- barrier + cooperative palette load: 0.5–1 ms
- expected total: 5–7 ms

Observed (after 40f3864 metal recorder dedup): 14–15 ms.

Gap factors (per Workflow `cpu-16ms-iterate` audit):
1. 100 dispatch batches (one per loaded GLB) → threadgroup barrier
   cost across many small groups + cache miss across 100 distinct pos
   and skin_attr buffer allocations.
2. Warp divergence in palette load loop
   (`assets/skin.metal:52-58`): every thread iterates, including
   lanes that early-exit at line 61 for vid ≥ vertex_count.
3. Dependent op chain in mat4 blend (`assets/skin.metal:97`):
   mx*w.x + my*w.y + mz*w.z + mw*w.w is a four-deep dependency.

Changes that landed this round:

- 19e9149 — present_queue_ deque drains in submit order (fixes
  the FPS regression from non-blocking try-present).
- 40f3864 — 16-slot linear cache for GetMtlBuffer resolutions in
  `DispatchSkinBatches` (metal). 100 batches × 2 lookups → ~N unique
  per frame.
- 984dae1 — `BuildSkinFrame` caches visible actors in
  `kept_entities[]` during pass 1; passes 2/3 iterate the flat array
  and skip the redundant 8-corner frustum recompute. skin_eval
  dropped 3.89 → 2.18 ms in debug; 0.83 ms in release.

GPU-side wins on this vertex count without an architectural change
are exhausted; further drop requires LOD or a single-dispatch
indirection table (both architectural).
