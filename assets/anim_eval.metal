...
//   buffer 1  : SceneHeader[]
//   buffer 2  : int parent[]
//   buffer 3  : int topo[]
//   buffer 4  : GpuTRS bind_pose[]
//   buffer 5  : GpuChannel channels[]
//   buffer 6  : GpuSampler samplers[]
//   buffer 7  : float times[]
//   buffer 8  : float4 values[]
//   buffer 9  : int joint_nodes[]
//   buffer 10 : float4x4 inverse_binds[]
//   buffer 11 : float4x4 world_scratch[]  (RW)
//   buffer 12 : float4x4 palette_out[]    (W)
//
...
static uint upper_key(const device float* times, uint times_off, uint count, float t) {
    uint lo = 0u;
    uint hi = count - 1u;
    while (lo + 1u < hi) {
        uint mid = (lo + hi) >> 1u;
...
static float4 sample_sampler(GpuSampler s,
    ...
    uint hi = upper_key(times, s.times_off, s.count, t);
    uint lo = hi - 1u;
...
kernel void anim_eval(uint3 gid [[thread_position_in_grid]],
                      ...
                      const device SceneHeader* headers  [[buffer(1)]],
                      const device int* parent           [[buffer(2)]],
                      const device int* topo             [[buffer(3)]],
                      const device GpuTRS* bind_pose     [[buffer(4)]],
                      const device GpuChannel* channels  [[buffer(5)]],
                      const device GpuSampler* samplers  [[buffer(6)]],
                      const device float* times          [[buffer(7)]],
                      const device float4* values        [[buffer(8)]],
                      const device int* joint_nodes      [[buffer(9)]],
                      const device float4x4* inverse_binds [[buffer(10)]],
                      device float4x4* world_scratch     [[buffer(11)]],
                      device float4x4* palette_out       [[buffer(12)]],
    ...
    for (uint c = tid; c < sh.channel_count; c += 64u) {
        ...
        float4 v = sample_sampler(s, ch.path, wrapped, times, values);
