...
// Shadow map (slot 3): combined depth sampler. Only sampled when
// inLightDir.w > 0 (shadow_strength == cast_shadows); set 3 is unbound
// otherwise, so the guard must gate the access.
...
// 3x3 PCF. NDC.z is [0,1] (ortho ZO) matching D32F; constant + slope bias
// against acne. NOTE: shadow-map UV Y convention vs the vk negative-height
// viewport needs interactive visual confirmation (see the M2b commit).
...
void main() {
    ...
    // Ambient is fill (unshadowed); only the direct term is occluded.
    vec3 lighting =
